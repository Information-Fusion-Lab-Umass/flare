...
class Tadpole_No_BN(nn.Module):
    ...
    def forward(self, x):
        ...
        print('Max after first affine: ', np.max(x.cpu().detach().numpy()))

        print('Min after first affine: ', np.max(x.cpu().detach().numpy()))
        x = self.dp2((F.relu(self.aff2(x))))
        print('Max after second affine: ', np.max(x.cpu().detach().numpy()))

        print('Min after second affine: ', np.max(x.cpu().detach().numpy()))
        x = self.dp3((F.relu(self.aff3(x))))
        print('Max after third affine: ', np.max(x.cpu().detach().numpy()))
