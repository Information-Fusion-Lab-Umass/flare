...
class ANN_DX_no_bn(nn.Module):
    ...
    def forward(self, x):
        def layer(x, fc, dp):
            return dp(F.relu(fc(x)))
        
        x = layer(x, self.fc1, self.dp1)
        x = layer(x, self.fc2, self.dp2)
        x = layer(x, self.fc3, self.dp2)
        x = layer(x, self.fc4, self.dp2)
        x = layer(x, self.fc5, self.dp1)
        x = layer(x, self.fc6, self.dp1)
        x = self.fc7(x)
