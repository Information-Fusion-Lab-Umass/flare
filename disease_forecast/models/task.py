...
class ANN_DX(nn.Module):
    ...
    def forward(self, x):
        def layer(x, fc, dp, n):
            x = F.relu(fc(x))
            return dp(nn.BatchNorm1d(n)(x))
        
        x = layer(x, self.fc1, self.dp1, 400)
        x = layer(x, self.fc2, self.dp2, 400)
        x = layer(x, self.fc3, self.dp2, 400)
        x = layer(x, self.fc4, self.dp1, 100)
        x = F.relu(self.fc5(x))
        return x
